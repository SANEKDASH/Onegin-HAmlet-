...
static int BinSearch(Lines *data,
                     Lines *item_pos,
                     int low,
                     int high,
...
void BinInsSort(Lines* data, int n, int (*CompFunc)(const void* str_lhs, const void* str_rhs))
{
    int i = 0, loc = 0, j = 0;
    Lines selected = {};
    ...
    for (i = 1; i < n; ++i)
    {
        ...
        loc = BinSearch(data, &selected, 0, j, CompFunc);
                     ...
                     int (*CompFunc)(const void* str_lhs, const void* str_rhs))
{
    if (high <= low)
    {
        return (CompFunc(item_pos, (data + low)) > 0 ) ? (low + 1) : low;
    }

    int mid = (low + high) / 2;

    if (CompFunc(item_pos, (data + mid)) == 0)
    {
        return mid + 1;
    }
    ...
    if (CompFunc(item_pos, (data + mid)) > 0)
    {
        return BinSearch(data, item_pos, mid + 1, high, CompFunc);
    }

    return BinSearch(data, item_pos, low, mid - 1, CompFunc);
}
// #defines!!!
